...
(function ($) {
  const app = {
    ...
    handleContactForm7PreviewTemplates: function () {
      // Handle Show Code button click
      const $showButtons = $(".templates-show-buttons");
      ...
      if ($showButtons.length) {
        $(".templates-show-buttons").on("click", function () {
          ...
          if ($formCodeContainer.is(":visible")) {
            ...
            $showButton
              .show()
              .html('<i class="fa-solid fa-code"></i> Show Code');
          } else {
            $formCodeContainer.show();
            $eacf7Form.hide();
            $copyButton.show();
      ...
      $(document).on("click", ".templates-form-code code", function () {
        ...
        copyToClipboard(
          codeText,
          $(this).closest(".form-area").find(".templates-copy-buttons")
        );
      });
      ...
      function fallbackCopyToClipboard(text, $copyButton) {
        ...
        try {
          ...
          if (successful) {
            ...
            alert(
              "Copy failed. Please try again or select text and copy manually."
            );
          }
        } catch (err) {
          console.error("Unable to copy text: ", err);
          alert(
            "Unable to copy text. Please select the text and copy manually (Ctrl+C)."
          );
        } finally {
          document.body.removeChild(textArea);
        }
