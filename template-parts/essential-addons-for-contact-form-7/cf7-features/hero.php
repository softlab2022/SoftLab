<section class="cf7-features-hero-area">
    <div class="container">
        <div class="row align-items-center">

            <div class="col-lg-6 col-md-6">
                <div class="hero-content">
                    <h1>Features of Essential Addons for Contact Form 7</h1>

                    <?php if (wp_is_mobile()) { ?>
                        <div class="hero-image-mobile text-center mb-5">
                        <img src="<?php echo get_template_directory_uri(); ?>/assets/images/cf7-extended/features/feat-hero-bg.png" alt="" class="img-fluid w-75 w-md-100 ">
                        </div>
                    <?php } ?>

                    <p>
                    Add advanced features, functionality, and customization options to your Contact Form 7 plugin with Essential Addons for Contact Form 7
                    </p>
                    <a href="/products" class="btn btn-primary view-products-btn"> <span></span> View Demo</a>

                </div>
            </div>

            <div class="col-lg-6 col-md-6">
                <div class="hero-image text-center text-md-end mt-5 mt-md-0">
                    <img src="<?php echo get_template_directory_uri(); ?>/assets/images/cf7-extended/features/feat-hero-bg.png" alt="" class="img-fluid w-75 w-md-100 ">
                </div>

            </div>

        </div>

    </div>
</section>